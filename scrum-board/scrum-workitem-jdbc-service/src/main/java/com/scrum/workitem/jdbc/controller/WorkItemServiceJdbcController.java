package com.scrum.workitem.jdbc.controller;
...
import com.scrum.workitem.jdbc.ScrumWorkItemJdbcServiceConfiguration;
import com.scrum.workitem.jdbc.dao.WorkitemAuditDaoImpl;
import com.scrum.workitem.jdbc.dao.WorkitemDaoImpl;

@RestController
public class WorkItemServiceJdbcController {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(WorkItemServiceJdbcController.class);
    ...
    public @ResponseBody DeferredResult<Boolean> createWorkItem(@RequestBody WorkitemModel workItemModel) {
    	...
    	CompletableFuture.supplyAsync(() -> { return workitemDaoImpl.createWorkitem(workItemModel); }, 
    									ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
    ...
    public @ResponseBody DeferredResult<Boolean> deleteWorkItem(@RequestBody WorkitemModel workItemModel) {
    	...
    	CompletableFuture.supplyAsync(() -> { return workitemDaoImpl.deleteWorkitem(workItemModel); }, 
    									ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
        .whenCompleteAsync((result, throwable) -> {
        			if(null != throwable){
        				deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(throwable.getMessage()));
    ...
    public @ResponseBody DeferredResult<Boolean> updateWorkItemStatus(@RequestBody WorkitemModel workItemModel) {
		LOGGER.info("received request to update workitem status with arguments : {}",workItemModel);
        DeferredResult<Boolean> deferredResult = new DeferredResult<Boolean>();
    	deferredResult.onTimeout(() -> {
    		deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.REQUEST_TIMEOUT).body("Request timeout occurred."));
    	});
    	CompletableFuture.supplyAsync(() -> { return workitemDaoImpl.updateWorkitem(workItemModel); }, 
    									ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
        .whenCompleteAsync((result, throwable) -> {
        			if(null != throwable){
        				deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(throwable.getMessage()));
    ...
    public @ResponseBody DeferredResult<List<WorkitemModel>> getAllWorkItems() {
    	...
    	CompletableFuture.supplyAsync(() -> { return workitemDaoImpl.getAllWorkitems(); }, 
    									ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
        .whenCompleteAsync((result, throwable) -> {
        			if(null != throwable){
        				deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(throwable.getMessage()));
	...
	public @ResponseBody DeferredResult<WorkitemModel> getWorkItemById(@PathVariable String id) {
		...
		CompletableFuture.supplyAsync(() -> { return workitemDaoImpl.getWorkitem(id); }, 
				ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
		.whenCompleteAsync((result, throwable) -> {
			if(null != throwable){
				deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(throwable.getMessage()));
	...
	public @ResponseBody DeferredResult<List<WorkitemAuditModel>> getWorkitemAuditById(@PathVariable String id) {
		...
		CompletableFuture.supplyAsync(() -> { return workitemAuditDaoImpl.getAllWorkitemAudit(id); }, 
				ScrumWorkItemJdbcServiceConfiguration.SCRUM_WORKITEM_EXECUTOR)
		.whenCompleteAsync((result, throwable) -> {
			if(null != throwable){
				deferredResult.setErrorResult(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(throwable.getMessage()));
